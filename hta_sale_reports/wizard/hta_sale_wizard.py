# -*- coding: utf-8 -*-
#################################################################################
# Author      : Halltech Africa. Ltd. (<www.halltech-africa.com>)
# 
...
class hta_sale_wizard(models.TransientModel):
    _name = "hta.sale.wizard"
    ...
    def generate_pdf_report(self):
        ...
        return self.env.ref('hta_sale_report.action_report_hta_sale').report_action(self, data=datas)

    @api.multi
    def generate_xls_report(self):
        self.check_date_range()
        report_hta_sale_obj = self.env['report.hta_sale_reports.hta_sale_template']
        ...
        for warehouse in self.warehouse_ids:
            ...
            if not self.group_by_categ:
                product_ids = report_hta_sale_obj._get_products(self)
                for product in product_ids:
                    beginning_qty = report_hta_sale_obj._get_beginning_inventory(self, product, warehouses=warehouse)
                    product_val = report_hta_sale_obj.get_product_sale_qty(self, product, warehouses=warehouse)
                ...
                rows += 1
                product_val = report_hta_sale_obj.get_product_sale_qty(self, warehouses=warehouse)
                if product_val:
                    for categ, product_value in product_val.items():
                        ...
                        for product in product_value:
                            ...
                            beginning_qty = report_hta_sale_obj._get_beginning_inventory(self, product_id.id,
