...
class HtaPartnerLedger(models.AbstractModel):
    _name = "account.report.hta.partner.ledger"
    _inherit = "account.report"
    _description = "customer partner ledger"
    ...
    def _query_group_by_account(self, options, line_id):
        ...
        return dict((res['partner_id'], res) for res in query_res)#dictionary of dictionary example 783: {'partner_id': 783, 'debit': 90000.0, 'credit': 0.0, 'balance': 90000.0},}

    
    def _get_group_by_partner_id(self, options, line_id):
        partners = {}
        account_types = [a.get('id') for a in options.get('account_type') if a.get('selected', False)]
        if not account_types:
            account_types = [a.get('id') for a in options.get('account_type')]
        date_from = options['date']['date_from']
        results = self._query_group_by_account(options, line_id)
        initial_bal_results = self.with_context(date_from=False, date_to=fields.Date.from_string(date_from) + timedelta(days=-1))._query_group_by_account(options, line_id)
    ...
    def _get_lines(self, options, line_id=None):
        ...
        grouped_partners = self._get_group_by_partner_id(options, line_id)
    ...
    def _get_report_name(self):
        return _('Custom Partner Ledger')
